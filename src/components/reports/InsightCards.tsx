...
function GrowthBadge({ pct }: { pct: number | null }) {
  ...
  if (pct < 0) return (
    <span className="flex items-center gap-1 text-red-500 dark:text-red-400 text-xs font-semibold">
      <TrendingDown size={12} /> {pct.toFixed(1)}%
    </span>
  )
  return <span className="flex items-center gap-1 text-muted-foreground text-xs"><Minus size={12} /> 0%</span>
}

function InsightCard({ item }: { item: InsightItem }) {
  const map = {
    warning: { icon: <AlertTriangle size={14} />, bg: 'bg-amber-50 dark:bg-amber-500/10', text: 'text-amber-600 dark:text-amber-400', border: 'border-amber-100' },
    success: { icon: <CheckCircle size={14} />,  bg: 'bg-green-50 dark:bg-green-500/10',  text: 'text-green-600 dark:text-green-400',  border: 'border-green-100' },
    info:    { icon: <Info size={14} />,         bg: 'bg-blue-50 dark:bg-blue-500/10',   text: 'text-blue-600 dark:text-blue-400',   border: 'border-blue-100' },
  }
