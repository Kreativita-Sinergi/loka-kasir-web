...
export default function ShiftScheduleFormModal({ schedule, open, onClose, onSuccess }: Props) {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { toast.success(t('scheduleCreated')); qc.invalidateQueries({ queryKey: ['shift-schedules'] }); onSuccess() },
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { toast.success(t('scheduleUpdated')); qc.invalidateQueries({ queryKey: ['shift-schedules'] }); onSuccess() },
    onError: (err) => toast.error(getErrorMessage(err)),
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.name.trim()) { toast.error(t('scheduleNameRequired')); return }
    ...
    <Modal open={open} onClose={onClose} title={schedule ? t('scheduleEdit') : t('scheduleAdd')} size="sm">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          <div>
            <span className="text-sm text-foreground">{t('scheduleOvernight')}</span>
            <p className="text-xs text-muted-foreground">{t('scheduleOvernightHint')}</p>
          </div>
        </label>
        ...
        {schedule && (
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={form.is_active} onChange={(e) => set('is_active', e.target.checked)}
              className="w-4 h-4 rounded border-border text-blue-600 dark:text-blue-400 focus:ring-blue-500" />
            <span className="text-sm text-foreground">{t('scheduleActive')}</span>
          </label>
        )}
