  ...
  return (
    <Modal open={open} onClose={onClose} title={schedule ? 'Edit Jadwal Shift' : 'Tambah Jadwal Shift'} size="sm">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-xs font-medium text-foreground mb-1">Nama Jadwal <span className="text-red-500 dark:text-red-400">*</span></label>
        ...
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          <div>
            <span className="text-sm text-foreground">Melewati Tengah Malam</span>
            <p className="text-xs text-muted-foreground">Centang untuk shift yang selesainya hari berikutnya (misal: 22:00 → 06:00)</p>
          </div>
        </label>
        ...
        {schedule && (
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" checked={form.is_active} onChange={(e) => set('is_active', e.target.checked)}
              className="w-4 h-4 rounded border-border text-blue-600 dark:text-blue-400 focus:ring-blue-500" />
            <span className="text-sm text-foreground">Jadwal Aktif</span>
          </label>
        )}
