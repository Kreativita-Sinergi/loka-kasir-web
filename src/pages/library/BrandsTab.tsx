...
export default function BrandsTab() {
  ...
  const columns = [
    { key: 'name', label: 'Nama Brand', render: (row: Brand) => <span className="font-semibold text-foreground">{row.name}</span> },
    {
      key: 'actions', label: '',
      render: (row: Brand) => (
    ...
    <>
      <div className="bg-card rounded-2xl border border-border">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <span className="text-sm text-muted-foreground">{pagination?.total ?? 0} Brand</span>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Edit Brand' : 'Tambah Brand'} size="sm">
        <form onSubmit={(e) => { e.preventDefault(); if (editing) updateMut.mutate(); else createMut.mutate() }} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">Nama Brand</label>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nike, Unilever, dll..." required className="w-full px-3 py-2.5 border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Hapus Brand" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">Yakin Ingin Menghapus Brand Ini?</p>
