...
export default function OrderTypesPage() {
  ...
  const columns = [
    { key: 'id', label: 'ID', render: (row: OrderType) => <span className="text-xs text-muted-foreground">#{row.id}</span> },
    { key: 'code', label: 'Kode', render: (row: OrderType) => <span className="font-mono text-xs font-semibold text-blue-600 dark:text-blue-400">{row.code}</span> },
    { key: 'name', label: 'Nama', render: (row: OrderType) => <span className="font-medium text-foreground">{row.name}</span> },
    {
      key: 'actions', label: '',
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="bg-card rounded-2xl border border-border">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            <span className="text-sm text-muted-foreground">{pagination?.total ?? 0} Tipe Order</span>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Hapus Tipe Order" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">Yakin Ingin Menghapus Tipe Order Ini?</p>
          <div className="flex gap-3">
