...
export default function CloseShiftModal({ open, shiftId, cashierId, pendingCount, onClose, onClosed }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    // Reset tampilan saat modal dibuka — setState sinkron disengaja di sini.
    ...
    <Modal open={open} onClose={onClose} title="Tutup Shift" size="sm">
      <div className="space-y-4">
        ...
        {loadingShift ? (
          ...
          // Tidak ada shift aktif (mis. sudah ditutup di aplikasi/perangkat
          // lain) → tak ada yang perlu ditutup di sini.
          <>
            <div className="flex items-start gap-2 rounded-xl bg-muted/40 px-3 py-3 text-sm text-muted-foreground">
              <CheckCircle2 size={16} className="mt-0.5 shrink-0 text-green-600 dark:text-green-400" />
              <span>
                Shift sudah ditutup (mungkin dari aplikasi atau perangkat lain).
                Tidak ada shift aktif untuk ditutup di sini.
              </span>
            </div>
            <Button className="w-full" onClick={onClosed}>
              Mengerti
            </Button>
          </>
        ) : (
          <>
            ...
            <div className="space-y-1.5 rounded-xl border border-border bg-muted/30 p-3 text-sm">
              ...
              <SummaryRow label="Kas Awal" value={formatCurrency(shift.opening_cash ?? 0)} />
              <SummaryRow
